...
class Cross_Attention(nn.Module):
    ...
    def forward(self, x, text):
        B, N, C = x.shape  #batch_size,196,64
        q = self.q(x)  # [B, num_patches, 3*embed_dim]
        q = q.reshape(B, N, self.num_heads, C // self.num_heads).permute(0,2,1,3)  # [B, num_patches, 3, num_heads, per_HeadDim]
        
        kv = self.kv(text)  # [3, B, num_heads, num_patches, per_HeadDim]
        kv = kv.reshape(B,-1,2,self.num_heads, C // self.num_heads).permute(2,0,3,1,4)
        k, v = kv[0], kv[1]  # [B, num_heads, num_patches, per_HeadDim] [4, 8, 196, 8/16/32/64] easy to use tensor
...
class upSkip(nn.Module):
    def __init__(self,in_channels):
         ...
         self.conv_layer = nn.Conv2d(in_channels=in_channels, out_channels=64, kernel_size=1)
         self.conv_after_up = nn.Conv2d(64, 64, kernel_size=3, padding=1)
         self.relu = nn.ReLU()

    def forward(self, x):
        output_tensor = self.conv_layer(x)
        output = self.upsample(output_tensor)
        output = self.relu(self.conv_after_up(output))
        return output


class gray_Cross_Attention(nn.Module):
    def __init__(self, config,patch_size,dim=64,img_size = 224,in_channels=64):
        super().__init__()
        self.Cross_Attention = Cross_Attention(dim = dim)
        self.embeddings = Embeddings(config=config, patch_size=patch_size, img_size=img_size, in_channels=in_channels)  
...
class VisionTransformer(nn.Module):  # Transformer-branch
    def __init__(self, config, vis, img_size, channel_num, patch_size, embed_dim, depth=1, num_heads=8,
        ...
        self.Cross_Encoder_blocks = Cross_Attention(dim = embed_dim,num_heads=num_heads,qkv_bias=qkv_bias,attn_drop=attn_drop_rate,proj_drop=drop_rate)
        ...
        self.CTBN3 = ConvTransBN(in_channels=70, out_channels=196)

    def forward(self, x, skip_x, text, reconstruct=False):
        
        x = self.embeddings(x) 
        if self.dim == 64:  
            text = self.CTBN3(text) 
            x = self.Cross_Encoder_blocks(x,text) 
