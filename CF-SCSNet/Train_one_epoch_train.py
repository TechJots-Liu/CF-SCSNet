...
#=================================================================================
##################################################################################
def train_one_epoch_Train(loader, model, criterion, optimizer, writer, epoch, lr_scheduler, model_type, logger):
    ...
    for i, (sampled_batch, names) in enumerate(loader, 1):
        ...
        images, masks,cu_masks, text = sampled_batch['image'], sampled_batch['label'],sampled_batch['cu_label'],  sampled_batch['text']
        if text.shape[1] > 70:
            text = text[ :, :70, :]
        
        images, masks,cu_masks, text = images.cuda(), masks.cuda(),cu_masks.cuda(), text.cuda()

        preds,preds_cu = model(images, text)
        pred_loss = criterion(preds, masks.float())  # Loss
        cu_loss = criterion(preds_cu,cu_masks.float())

        out_loss = 1*pred_loss+0.2*cu_loss
        ...
        if i % config.print_frequency == 0:
            train_lr = min(g["lr"] for g in optimizer.param_groups)
